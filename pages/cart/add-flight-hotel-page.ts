...
export class AddFlightHotelPage extends Page {
  ...
  async createFlightHotel(
    ...
    if (depDayFlight !== null) {
      ...
      log.warn("Skipping return date selection because departure date failed.");
    }
    ...
    if (await summaryProceedBtn.isExisting()) {
      ...
      if (await chooseAncillariesScreen.isExisting()) {
        ...
        if (await ancillariesProceedBtn.isExisting()) {
          log.info("Proceed button on Choose Ancillaries found, clicking...");
    ...
    log.info("PROCEED BUTTON CLICKED AND CREATE TRAVEL REQUEST SCREEN LOADED");
    await driver.pause(4000);
    ...
    await hotelSearchingScreenLoading
      ...
      .catch(() => {
        log.info("HOTEL SEARCHING LOADER  NOT VSISBLE .");
      });
    ...
    log.info("HOTEL SEARCHING RESULT SCREEN CLICKED FOUND");
